...
function App() {
  ...
  const makeSegments = () => {
    ...
    return ingredients.map((ingredient, index) => { // map goes through each ingredient in the array
      const colour = colours[0] // cycles through colours
      if(noIngredients % 6 === 1){
        // const colour = colours[Math.floor(Math.random()*3+1)]
        alert("!!!!!!!!!!!!!")
      }
      else {
      ...
      return (
        <g key={index} transform={`rotate(${angleAdded})`}>
          ...
          <text
            x="50"
            y="10"
            transform={`rotate(${angle / noIngredients}, 50, 10)`}
            
            //  transform={`rotate(${angle / 2 +180/noIngredients}, 50, 10)`}
